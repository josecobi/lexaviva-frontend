...
function IllustrationGrid({imgResults}){
    ...
    const handleIllustrationClick = (src, author) => {
        // Check if the illustration is already selected
        if(selectedIllustration.src === src && selectedIllustration.author === author) {
            // If it is, deselect it
            console.log(`img from ${author} deselected`)
            return dispatch(deSelectIllustration());
        }
        // If it is not, select it
        console.log(`img from ${author} selected`)
        dispatch(selectIllustration({src, author}));
    }
    return(
        <div className="grid">
            {   imgResults.map((img) => {           
                    ...
                    return <Illustration onClick={() => (handleIllustrationClick(src, author))} key={imgId} imgId={imgId} src={src} author={author} />
                }           
            )}   
